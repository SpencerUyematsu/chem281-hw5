...
int count_intervening_ones (const bitset<N> &b, int l, int r)
{
    ...
    int max_position = std::max(l, r);
    int min_position = std::min(l, r);
    int count = 0;
...
template <size_t N>
int count_intervening_ones (const std::bitset<N> &b, int l, int r)
{
	// Leave for students to implement
    int max_position = std::max(l, r);
    int min_position = std::min(l, r);
...
template <size_t N>
bool is_neg (const std::bitset <N> &ket, int i, int a ) { return count_intervening_ones (ket,i,a)%2; }

template <size_t N>
bool is_neg (bitset <N> ket, int i, int j, int a, int b)
{
	const bool neg_ai = is_neg (ket, i,a);

    // std::cout << ket.flip(i).flip(a) << std::endl;
		
    const bool neg_bj = is_neg (ket.flip(i).flip(a), j,b);
